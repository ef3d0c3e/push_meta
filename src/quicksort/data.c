...
{
	plot.tagged = NULL;
	plot.tagged_size = plot.tagged_capacity = 0;
	data->plots = xrealloc(data->plots, sizeof(plot) * (data->plots_size + 1));
	data->plots[data->plots_size] = plot;
	return &data->plots[data->plots_size++];
}

void
quicksort_plot_add_value(quicksort_plot_t* plot, char* value)
{
	if (plot->tagged_size >= plot->tagged_capacity) {
		const size_t new_cap = plot->tagged_capacity * 2 + !plot->tagged_capacity * 16;
		plot->tagged = xrealloc(plot->tagged, sizeof(char*) * new_cap);
		plot->tagged_capacity = new_cap;
	}
	plot->tagged[plot->tagged_size++] = value;
}
...
quicksort_data_free(quicksort_data_t* data)
{
	for (size_t i = 0; i < data->plots_size; ++i) {
		...
		for (size_t j = 0; j < data->plots[i].tagged_size; ++j) {
			free(data->plots[i].tagged[j]);
		}
		free(data->plots[i].tagged);
	}
	free(data->plots);
}
...
quicksort_write_plots(const quicksort_data_t* data)
{
	for (size_t i = 0; i < data->plots_size; ++i) {
		...
		fwrite("\n", 1, 1, file);
		for (size_t i = 0; i < plot->tagged_size; ++i) {
			fwrite("TAG,", 1, 4, file);
			fwrite(plot->tagged[i], 1, strlen(plot->tagged[i]), file);
			fwrite("\n", 1, 1, file);
		}
